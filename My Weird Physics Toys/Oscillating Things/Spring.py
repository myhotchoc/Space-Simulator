...
class Spring(object):
    def __init__(self, k, rest_length, origin):
        self.k = k
        self.rest_length = rest_length
        self.origin = origin
        
    
    def start(self, body, stretch):
        body.px = self.origin[0]
        body.py = self.origin[1] + self.rest_length + stretch
    
    def connect(self, body):
        stretch = body.py - (self.rest_length + self.origin[1])
        
        force = -self.k * stretch
        
        #print (force)
        
        body.applyForce((0, force))
        pygame.draw.line(screen, WHITE, self.origin, (body.px, body.py), 2)
        #print (force, body.ay)
        print('x=',stretch)
    
    def limit(self, body):
        if body.py < self.origin[1]:
            body.py = self.origin[1]+1
        if body.py > 800:
            body.py = 799
        
    
        
body1 = Body(RED, 50, (centre[0], 0),
             (0, 0),
             (0, 0),
             (0, 0))
...
s1 = Spring(0.1, 200, (centre[0], 300))

...
class Body(object):
    ...
    def changePos(self):
        ...
        ## Amplitude * sin(angle)
        self.px = centre[0] + int(self.ampx * math.sin(self.thetax))
        self.py = centre[1] + int(self.ampy * math.sin(self.thetay))
...
## Five predefined objects
        
# =============================================================================
# body1 = Body(RED, 70, centre,
#              (0, 0),
#              (0, 0),
#              (0, 0),
#              (200, 250),
...
## Define array of Body objects
bodies = []

## Make 30 random objects
for i in range(20):
    b = Body((random.randint(0,255), random.randint(0,255), random.randint(0,255)), #colour
             30, centre, ## radius, screen
              (0, 0), # vel, acc, force
              (0, 0),
              (0, 0),
              (random.randint(100, 400), random.randint(100, 400)), ## amplitude
              (0, 0), ## Initial angle
              (random.uniform(0.02, 0.08), random.uniform(0.02, 0.08))) ## frequency
    
    bodies.append(b)

## To make objects draw out path, put screen.fill(BLACK) OUTSIDE the mainloop
